...
- (void)performNativeMethod {
    ...
    self.block = ^(NSDictionary *argsDic, BOOL success) {
    ...
    if (_responder && self.nativeMethod && self.nativeMethod.length) {
        if ([_responder respondsToSelector:NSSelectorFromString(self.nativeMethod)]) {
          id result = [_responder performSelector:NSSelectorFromString(self.nativeMethod) withObject:self.nativeDict withObject:self.block];
            if (result) {
...
+ (NSString *)converDicToJsonStr:(NSDictionary *)dic {
